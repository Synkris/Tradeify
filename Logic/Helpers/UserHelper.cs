...
using static Core.Config.PaystackResponse;
using X.PagedList;
...
namespace Logic.Helpers
{
    public class UserHelper : IUserHelper
    {
        ...
        public UserHelper(AppDbContext context, UserManager<ApplicationUser> userManager, SignInManager<ApplicationUser> signInManager, IGeneralConfiguration generalConfiguration, IEmailService emailService, IEmailHelper emailHelper, IHttpContextAccessor httpContext)
        {
            _context = context;
        ...
        public List<RolesViewModel> GetRoles()
        {
            ...
            var rolesViewModelList = roles.Select(role => new RolesViewModel
            {
        ...
        public string GetUserRole(ApplicationUser user)
        {
            if (user != null)
            {
                var getUserRole = _userManager.GetRolesAsync(user).Result;
                if (getUserRole.Any())
                {
                    return JsonConvert.SerializeObject(getUserRole);
                }
            }
            return null;
        }
        ...
        public async Task<Impersonation> GetLatestImpersonateeRecord(string userId)
        {
            try
            {
                ...
                {
                    //var impersonationRecords = _context.Impersonations.Where(x => x.DateImpersonated == lastTimeImpersonated).FirstOrDefault();
        ...
        public bool CheckRefRegPayment(string refferrerId)
        {
            if (refferrerId != null)
            {
                var refPay = _context.ApplicationUser.Where(s => s.Id == refferrerId && (!s.Deactivated || s.DateRegistered.Year < 2024 )&& s.RegFeePaid == true).FirstOrDefault();
                if (refPay != null)
                {
                    return true;
                }
        ...
        public bool EditNews(News newsDetails)
        {
            try
            {
                if (newsDetails != null)
                {
                    var news = _context.News.Where(e => !e.Deleted && e.Id == newsDetails.Id).FirstOrDefault();
                    if (news != null)

                    {
                        news.Name = newsDetails.Name;
                        news.DateCreated = DateTime.Now;
                        news.Details = newsDetails.Details;
                        news.MasterImageUrl = newsDetails.MasterImageUrl;
        ...
        public bool DeleteNews(News newsdetails)
        {
            try
            {
                if (newsdetails != null)
                {
                    var news = _context.News.Where(c => c.Id == newsdetails.Id && !c.Deleted).FirstOrDefault();
                    if (news != null)
                    {
                        news.Deleted = true;
                        news.Active = false;
        ...
        public bool UpdateMemberProfile(ApplicationUser applicationUser, string userName)
        {
            try
            {
                if (applicationUser != null)
                {
                    var staff = FindByUserName(userName);
                    if (staff != null)
                    {

                        staff.FirstName = applicationUser.FirstName;
                        staff.LastName = applicationUser.LastName;
                        staff.GenderId = applicationUser.GenderId;
                        staff.Email = applicationUser.Email;
                        staff.PhoneNumber = applicationUser.PhoneNumber;
                        staff.UserName = applicationUser.UserName;
                        staff.NormalizedUserName = applicationUser.UserName.ToUpper();
                        staff.NormalizedEmail = applicationUser.Email.ToUpper();
        ...
        public PendingBonuses GetUserPendingBonus(string username)
        {
            try
            {
                if (username != null)
                {
                    ...
                    if (currentUser != null)
                    {
                        var userBonus = _context.PendingBonuses.Where(x => x.UserId != null && x.UserId == currentUser.Id).FirstOrDefault();
                ...
                if (loggedInUser != null)
                {
                    ...
                    if (impersonatee != null)
                    {
                        ...
                        if (impersonationRecord != null)
                        {
                                ...
                                impersonation.EndDBSession = impersonationRecord.EndSession;
        ...
        public async Task<bool> CreditWallet(string userId, decimal amount)
        {
            ...
            try
            {
                if (userId != null && amount > 0)
                {
                    ...
                    else
                    {
                        ...
                        _context.SaveChanges();
                        var result = await LogUserMiningHistory(userId, amount);
                        if (result)
                            return true;
                    }
                }
        ...
        public decimal GetUserWallet(string userId)
        {
            if (userId != null)
            {
                var walletBalance = _context.Wallets.Where(x => x.UserId == userId).FirstOrDefault();
        ...
        public decimal GetPvWallet(string userId)
        {
            if (userId != null)
            {
                var pvBalance = _context.PvWallets.Where(x => x.UserId == userId).FirstOrDefault();
                if (pvBalance != null)
                {
                    return pvBalance.Balance;
                }
            }
            return 0;
        }
        public decimal GetAGCWallet(string userId)
        {
            if (userId != null)
            {
                var agcBalance = _context.AGCWallets.Where(x => x.UserId == userId).FirstOrDefault();
                if (agcBalance != null)
                {
                    return agcBalance.Balance;
                }
            }
            return 0;
        }
