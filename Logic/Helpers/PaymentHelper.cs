    ...
    public class PaymentHelper : BaseHelper, IPaymentHelper
	{
        ...
        public UserPackages GetUserPackage(string userId)
		{
			if (userId != null)
			{
				...
				{
					LogCritical($"An attempt to get user package with userId{userId} failed");
				}
			}
			return null;
		}
		...
		public bool CreateRegFee(PaymentFormViewModel paymentDetails)
		{
			...
			{
				LogCritical($" {ex.Message} This exception occured while trying to create user regfee with {paymentDetails.ToString()}");
				throw ex;
			}
		}
		...
		public PvWallet GetUserPvWalletNonAsync(string userId)
		{
			...
			{
				LogCritical($" {ex.Message} This exception occured while trying to get user pv wallet");
				throw ex;
			}
		}
		public PvWallet CreatePvWalletByUserIdNonAsync(string userId, decimal amount = 0)
		{
			...
			try
			{
				if (userId != null)
				{
					...
					{
						LogError($"An attempt to get user for creating pv wallet with userId{userId} failed");
					}
				}
				return null;
			}
			...
			{
				LogCritical($" {ex.Message} This exception occured while trying to create PvWallet");
				throw ex;
			}
		}
		...
		public Wallet CreateWalletNowAsynByUserId(string userId, decimal amount = 0)
		{
			try
			{
				if (userId != null)
		...
		public AGCWallet GetUserAGCWalletNonAsync(string userId)
		{
			...
			{
				LogCritical($" {ex.Message} This exception occured while trying to Get User AGCWalletnon async");
				throw ex;
			}
		}
		public AGCWallet CreateAGCWalletByUserIdNonAsync(string userId, decimal amount = 0)
		{
			...
			{
				LogCritical($" {ex.Message} This exception occured while trying to create user AGCWallet non async");
				throw ex;
			}
		}
		...
		public decimal GetReactivationAmount()
		{
			var getAmount = _context.CompanySettings.Where(x => x.Active).FirstOrDefault();
			if (getAmount != null)
			{
				return getAmount.ActivationAmount;
